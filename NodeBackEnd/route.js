...
var Coment = require('./coment_model')
...
router.get('/comments/:id',(req,res)=>{
    Coment.find({Blog_id:req.params.id},(err,docs)=>{
        if(err){
            res.json(err)
        }
        else{
...
router.delete('/comment/:id',(req,res)=>{
    Coment.findByIdAndDelete({_id:req.params.id},(err)=>{
        if(err)
        res.json(err);
        else
        res.json({msg:"delete Sucessfully"})
